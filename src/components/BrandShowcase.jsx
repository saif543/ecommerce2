...
export default function BrandShowcase() {
  ...
  if (loadingBrands) {
    return (
      <section className="bg-offwhite py-8 min-[768px]:py-14">
        <div className="max-w-[1440px] mx-auto px-3 min-[480px]:px-5 min-[768px]:px-6">
          ...
          <div className="flex gap-8 justify-center overflow-hidden mb-8">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="flex-shrink-0 w-20 h-8 bg-gray-200 rounded animate-pulse" />
            ))}
          </div>
          <div className="flex gap-4 overflow-hidden">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="flex-shrink-0 flex-1 h-64 bg-gray-100 rounded-xl animate-pulse" />
            ))}
          </div>
        </div>
      </section>
    ...
    <section className="bg-offwhite py-8 min-[768px]:py-14">
      <div className="max-w-[1440px] mx-auto px-3 min-[480px]:px-5 min-[768px]:px-6">
        ...
        <motion.h2
          ...
          viewport={{ once: true }}
          className="text-xl min-[480px]:text-2xl md:text-3xl font-semibold text-text-primary text-center mb-6 min-[768px]:mb-8"
        >
          Shop By Brands
        </motion.h2>

        {/* Brand Row — logos + arrows + Show All */}
        <div className="flex items-center gap-2 min-[480px]:gap-3 mb-6 min-[768px]:mb-8">
          {/* Left Arrow */}
          <button
            onClick={() => scrollBrands(-1)}
            className="flex-shrink-0 w-8 h-8 min-[768px]:w-9 min-[768px]:h-9 rounded-full border border-gray-300 flex items-center justify-center text-text-secondary hover:border-purple-mid hover:text-purple-mid transition-colors bg-white"
          >
            <ChevronLeft size={18} />
          </button>
          ...
          <div
            ref={brandScrollRef}
            className="flex-1 flex items-center gap-4 min-[480px]:gap-6 min-[768px]:gap-8 overflow-x-auto touch-pan-x"
                ...
                >
                  <div className="flex flex-col items-center gap-1.5">
                    {brand.logo ? (
                      ...
                      <div className={`w-14 h-14 min-[480px]:w-16 min-[480px]:h-16 min-[768px]:w-[72px] min-[768px]:h-[72px] rounded-xl border flex items-center justify-center ${isActive ? "bg-purple-soft border-purple-mid/20" : "bg-gray-50 border-gray-100"}`}>
                        <span className={`font-bold text-xl ${isActive ? "text-purple-mid" : "text-text-muted"}`}>
                          {brand.name?.[0]?.toUpperCase()}
                        </span>
                      </div>
                    )}
                    <span className={`text-[10px] min-[480px]:text-xs font-semibold whitespace-nowrap ${isActive ? "text-text-primary" : "text-text-muted"}`}>
                      {brand.name}
                    </span>
                  </div>
                  {/* Active underline */}
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute bottom-0 left-0 right-0 h-[2.5px] bg-purple-dark rounded-full"
          ...
          {activeBrand && (
            <Link
              ...
              className="flex-shrink-0 hidden min-[640px]:flex items-center gap-1 text-sm font-semibold text-purple-mid hover:text-purple-dark transition-colors ml-2"
            >
              Show All
            ...
            {loadingProducts ? (
              ...
              <div className="relative group">
                ...
                <button
                  ...
                  className="absolute left-0 top-[40%] -translate-y-1/2 z-10 w-9 h-9 min-[768px]:w-10 min-[768px]:h-10 bg-white border border-gray-200 rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity hover:shadow-lg hover:border-gray-300"
                >
                ...
                <div
                  ...
                  className="flex gap-2 min-[480px]:gap-3 min-[768px]:gap-4 overflow-x-auto pb-2 touch-pan-x"
                  style={{ scrollbarWidth: "none", msOverflowStyle: "none", WebkitOverflowScrolling: "touch" }}
                >
                  <style jsx>{`div::-webkit-scrollbar { display: none; }`}</style>
                ...
                <button
                  onClick={() => scrollProducts(1)}
                  className="absolute right-0 top-[40%] -translate-y-1/2 z-10 w-9 h-9 min-[768px]:w-10 min-[768px]:h-10 bg-white border border-gray-200 rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity hover:shadow-lg hover:border-gray-300"
                >
        ...
        {activeBrand && (
          <div className="text-center mt-5 min-[640px]:hidden">
            <Link
              href={`/brands/${toSlug(activeBrand.name)}`}
              className="inline-flex items-center gap-1.5 text-sm font-semibold text-purple-mid hover:text-purple-dark transition-colors"
            >
