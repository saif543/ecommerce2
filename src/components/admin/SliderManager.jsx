...
export default function SliderManagement({ getToken }) {
    ...
    const fetchSlides = async () => {
        try {
            ...
            MySwal.fire({ icon: 'error', title: 'Error', text: 'Failed to load slides', confirmButtonColor: '#F47B20' })
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleImageSelect = (e) => {
        ...
        if (!allowedTypes.includes(file.type)) {
            MySwal.fire({ icon: 'error', title: 'Invalid File Type', text: 'Only JPEG, PNG, WebP are allowed', confirmButtonColor: '#F47B20' })
            return
        }
        if (file.size > 100 * 1024 * 1024) {
            MySwal.fire({ icon: 'error', title: 'File Too Large', text: 'Maximum file size is 100MB', confirmButtonColor: '#F47B20' })
            return
        }
        setImageFile(file)
    ...
    const handleSaveSlide = async (e) => {
        ...
        if (!editingSlide && !imageFile) {
            MySwal.fire({ icon: 'warning', title: 'Image Required', text: 'Please select an image for the slide', confirmButtonColor: '#F47B20' })
            return
        }
        setSaving(true)
        try {
            ...
            MySwal.fire({
                ...
                timer: 1500, showConfirmButton: false, confirmButtonColor: '#F47B20',
            })
            setShowModal(false)
            setTimeout(() => fetchSlides(), 500)
        } catch (error) {
            console.error('Error saving slide:', error)
            MySwal.fire({ icon: 'error', title: 'Error', text: error.message || 'Failed to save slide', confirmButtonColor: '#F47B20' })
        } finally {
            setSaving(false)
            setUploading(false)
        }
    ...
    const handleDeleteSlide = async (slideId) => {
        ...
        try {
            ...
            MySwal.fire({ icon: 'success', title: 'Deleted!', text: 'Slide permanently deleted', timer: 1500, showConfirmButton: false, confirmButtonColor: '#F47B20' })
            fetchSlides()
        } catch (error) {
            MySwal.fire({ icon: 'error', title: 'Error', text: error.message || 'Failed to delete slide', confirmButtonColor: '#F47B20' })
        }
    }
    ...
    const handleToggleActive = async (slide) => {
        try {
            ...
            MySwal.fire({ icon: 'error', title: 'Error', text: 'Failed to update slide status', confirmButtonColor: '#F47B20' })
        }
    }
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    {slide.link ? (
                                                        <a
                                                            ...
                                                            className="text-sm text-[#F47B20] hover:underline truncate block"
                                                        >
                                                            {slide.link}
                                                        </a>
                                                    ) : (
            ...
            <AnimatePresence>
                {showModal && (
                    ...
                    >
                        ...
                        >
                            {/* Modal Header */}
                            <div className="bg-gradient-to-r from-[#F47B20] to-[#C45A00] text-white p-5 flex items-center justify-between">
                            ...
                            <form onSubmit={handleSaveSlide} className="p-6 space-y-5">
                                ...
                                <div>
                                    ...
                                    <div
                                        className="border-2 border-dashed border-gray-300 rounded-xl p-5 text-center cursor-pointer hover:border-[#F47B20] hover:bg-orange-50 transition-all"
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#F47B20]"
                                    />
                                </div>
                                ...
                                <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                                    <input
                                        ...
                                        className="w-5 h-5 rounded cursor-pointer accent-[#F47B20]"
                                    />
