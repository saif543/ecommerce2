...
export async function GET(req) {
    ...
    try {
        ...
        const cart = await db.collection('carts').findOne({ userEmail: user.email })

        // Return empty cart shape if none exists yet
        if (!cart) {
            return NextResponse.json({ cart: { userEmail: user.email, items: [], totalAmount: 0, updatedAt: null } })
        }
        ...
        return NextResponse.json({ cart: { ...cart, totalAmount } })
...
export async function POST(req) {
    ...
    try {
        ...
        if (!isValidPrice(price)) return NextResponse.json({ error: 'Valid price is required' }, { status: 400 })
        if (isNaN(quantity) || quantity < 1) return NextResponse.json({ error: 'Quantity must be at least 1' }, { status: 400 })
        if (quantity > MAX_QUANTITY_PER_ITEM) return NextResponse.json({ error: `Quantity cannot exceed ${MAX_QUANTITY_PER_ITEM}` }, { status: 400 })
        ...
        const existingIdx = existingItems.findIndex(i => i.productId === productId)

        if (existingIdx >= 0) {
            // Add quantity to existing item
            const newQty = Math.min(existingItems[existingIdx].quantity + quantity, MAX_QUANTITY_PER_ITEM)
            ...
            if (existingItems.length >= MAX_CART_ITEMS) {
                return NextResponse.json({ error: `Cart limit reached (max ${MAX_CART_ITEMS} items)` }, { status: 400 })
            }
            existingItems.push({ productId, name, price, quantity, image, addedAt: new Date() })
        }

        const totalAmount = parseFloat(existingItems.reduce((sum, i) => sum + i.price * i.quantity, 0).toFixed(2))

        await db.collection('carts').updateOne(
            { userEmail: user.email },
            { $set: { userEmail: user.email, userId: user.dbUserId || user.userId, items: existingItems, totalAmount, updatedAt: new Date() } },
            { upsert: true }
        )

        const updatedCart = await db.collection('carts').findOne({ userEmail: user.email })
        logAudit('CART_ITEM_ADDED', { userId: user.userId, userEmail: user.email, productId, quantity }, req)
...
export async function PUT(req) {
    ...
    try {
        ...
        if (!productId) return NextResponse.json({ error: 'productId is required' }, { status: 400 })
        if (isNaN(quantity) || quantity < 0) return NextResponse.json({ error: 'Valid quantity is required (0 to remove)' }, { status: 400 })
        if (quantity > MAX_QUANTITY_PER_ITEM) return NextResponse.json({ error: `Quantity cannot exceed ${MAX_QUANTITY_PER_ITEM}` }, { status: 400 })
        ...
        const totalAmount = parseFloat(items.reduce((sum, i) => sum + i.price * i.quantity, 0).toFixed(2))
        ...
        const updatedCart = await db.collection('carts').findOne({ userEmail: user.email })
        logAudit('CART_ITEM_UPDATED', { userId: user.userId, userEmail: user.email, productId, newQuantity: quantity }, req)
...
export async function DELETE(req) {
    ...
    try {
        ...
        if (!productId && !clearAll) {
            return NextResponse.json({ error: 'productId or clearAll=true is required' }, { status: 400 })
        }
        ...
        if (clearAll) {
            ...
            logAudit('CART_CLEARED', { userId: user.userId, userEmail: user.email }, req)
            return NextResponse.json({ message: 'Cart cleared', cart: { userEmail: user.email, items: [], totalAmount: 0 } })
        }
        ...
        const updatedCart = await db.collection('carts').findOne({ userEmail: user.email })
        logAudit('CART_ITEM_REMOVED', { userId: user.userId, userEmail: user.email, productId }, req)
