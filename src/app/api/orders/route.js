...
function sanitizeItems(items) {
  if (
    !Array.isArray(items) ||
    items.length === 0 ||
    items.length > MAX_ITEMS_PER_ORDER
  )
    return null;
  return items
    .map((item) => {
...
export async function GET(req) {
  ...
  try {
    ...
    const CACHE_KEY = `orders:${orderId}:${getAllOrders}:${status}:${page}:${limit}`;
    const CACHE_ORDERS = await getRedisData(CACHE_KEY);
    if (CACHE_KEY) {
      return NextResponse.json(CACHE_ORDERS, { status: 200 });
    }
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Valid delivery address required (fullName, phone, shippingZone, area, streetAddress)",
        },
        { status: 400 },
      );
    }
    ...
    if (!VALID_PAYMENT_METHODS.includes(paymentMethod)) {
      return NextResponse.json(
        {
          error: `Valid payment method required: ${VALID_PAYMENT_METHODS.join(", ")}`,
        },
        { status: 400 },
      );
    }
    ...
    const shippingCost =
      deliveryAddress.shippingZone === "inside"
        ...
        if (!expired && !usedUp) {
          ...
          if (coupon.scope === "all") {
            ...
            eligibleTotal = items
              .filter(
                (i) =>
                  coupon.categories?.includes(i.category) &&
          ...
          if (
            eligibleTotal > 0 &&
            eligibleTotal >= (coupon.minOrderAmount || 0)
          ) {
            ...
            couponDiscount = parseFloat(
              Math.min(couponDiscount, subtotal).toFixed(2),
            );
    ...
    if (couponDocId) {
      setImmediate(async () => {
        try {
          await db
            .collection("coupons")
            .updateOne({ _id: couponDocId }, { $inc: { usedCount: 1 } });
        } catch (e) {
    ...
    return NextResponse.json(
      { success: true, message: "Order placed successfully", order: created },
      { status: 201 },
    );
  } catch (err) {
...
export async function PUT(req) {
  ...
  try {
    ...
    const { ObjectId } = await import("mongodb");
    const body = await req.json();
    const { status, notes, userName, userEmail, userPhone, deliveryAddress } =
      body;
    ...
    if (status && !VALID_ORDER_STATUSES.includes(status)) {
      return NextResponse.json(
        { error: `Valid status required: ${VALID_ORDER_STATUSES.join(", ")}` },
        { status: 400 },
      );
    }
    ...
    if (notes !== undefined)
      updateData.adminNotes = sanitizeString(String(notes), 1000);

    // Optional Inline Edits (Customer & Address)
    if (userName !== undefined)
      updateData.userName = sanitizeString(String(userName), 100);
    if (userEmail !== undefined)
      updateData.userEmail = sanitizeString(String(userEmail), 100);
    if (userPhone !== undefined)
      updateData.userPhone = sanitizeString(String(userPhone), 20);

    if (deliveryAddress && typeof deliveryAddress === "object") {
      updateData.deliveryAddress = {
        ...order.deliveryAddress,
        fullName: sanitizeString(
          deliveryAddress.fullName || order.deliveryAddress?.fullName,
          100,
        ),
        ...
        phone: sanitizeString(
          deliveryAddress.phone || order.deliveryAddress?.phone,
          20,
        ),
        ...
        shippingZone: sanitizeString(
          deliveryAddress.shippingZone || order.deliveryAddress?.shippingZone,
          50,
        ),
        area: sanitizeString(
        ...
        streetAddress: sanitizeString(
          deliveryAddress.streetAddress || order.deliveryAddress?.streetAddress,
          500,
        ),
      };
    }
    ...
    if (status === "shipped" && order.status !== "shipped") {
      const bulkOps = order.items
        .map((item) => {
          ...
          if (!item.productId || isNaN(qtyToDeduct) || qtyToDeduct <= 0)
            return null;
    ...
    if (status) {
      ...
      logAudit(
        ...
        { userId: user.userId, userEmail: user.email, orderId: id },
        req,
      );
      return NextResponse.json(
        {
...
export async function DELETE(req) {
  ...
  try {
    ...
    if (user.role !== "admin") {
      if (order.userEmail !== user.email && order.userPhone !== user.phone)
        return createAuthError("Access denied", 403);
    ...
    if (hardDelete) {
      ...
      return NextResponse.json(
        { success: true, message: "Order permanently deleted" },
        { status: 200 },
      );
    }
