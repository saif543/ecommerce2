...
export async function PUT(req) {
    ...
    try {
        ...
        if (body.type === 'video') {
            ...
            await db.collection('homepage_video').updateOne(
                {},
        ...
        if (!body.id) {
            ...
            const newSection = {
                ...
                productLimit: Math.min(20, Math.max(4, parseInt(body.productLimit) || 8)),
                bg: body.bg || 'bg-white',
                sectionBannerKey: (body.sectionBannerKey || '').trim(),
                bannerGradient: (body.bannerGradient || 'linear-gradient(135deg, #111111 0%, #1a1a1a 40%, #222222 100%)').trim(),
        ...
        const allowedFields = ['order', 'title', 'subtitle', 'filterType', 'filterValue', 'productLimit', 'bg', 'sectionBannerKey', 'bannerGradient', 'seeAllLink', 'isActive']
        for (const field of allowedFields) {
            if (body[field] !== undefined) {
                updateData[field] = field === 'productLimit'
        ...
        const updated = await db.collection('homepage_sections').findOne({ id: body.id })
...
export async function DELETE(req) {
    ...
    try {
        ...
        if (result.deletedCount === 0) return NextResponse.json({ error: 'Section not found' }, { status: 404 })
