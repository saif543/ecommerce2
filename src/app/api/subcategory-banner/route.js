...
export async function GET(req) {
  try {
    ...
    const subcategory = sanitizeString(
    ...
    const CACHE_KEY = `subcategory_banners:${subcategory}`;
    const CACHE_SUBCATEGORY = await getRedisData(CACHE_KEY);
    if (CACHE_SUBCATEGORY) {
      return NextResponse.json(CACHE_SUBCATEGORY, { status: 200 });
    }
    ...
    const banners = await db
      .collection("subcategory_banners")
      .find({})
      .sort({ subcategory: 1 })
      .toArray();
...
export async function PUT(req) {
  ...
  try {
    ...
    if (!subcategoryDoc) {
      return NextResponse.json(
        { error: `subcategory "${subcategory}" not found in database` },
        { status: 404 },
      );
    }
    ...
    const uploaded = await uploadImage(buffer, {
      ...
      transformation: [
        { width: 1440, height: 176, crop: "fill", gravity: "auto" },
      ],
    });
    ...
    const updated = await db
      .collection("subcategory_banners")
      .findOne({ subcategory: subcategoryDoc.name });

    logAudit(
      "subcategory_BANNER_UPLOADED",
      { userId: user.userId, userEmail: user.email, subcategory },
      req,
    );
    return NextResponse.json({
      success: true,
      message: `Hero banner for "${subcategory}" uploaded successfully`,
      banner: updated,
    });
  } catch (err) {
...
export async function DELETE(req) {
  ...
  try {
    ...
    if (!existing) {
      return NextResponse.json(
        { error: "Banner not found for this subcategory" },
        { status: 404 },
      );
    }
    ...
    logAudit(
      "subcategory_BANNER_DELETED",
      { userId: user.userId, userEmail: user.email, subcategory },
      req,
    );
    return NextResponse.json({
      success: true,
      message: `Hero banner for "${subcategory}" removed successfully`,
    });
  } catch (err) {
