...
export async function GET(req) {
  try {
    ...
    const CACHED_CATEGORY = await getRedisData(CACHE_KEY);
    if (CACHED_CATEGORY) {
      console.log("[REDIS] CACHE HIT");
      return NextResponse.json(CACHED_CATEGORY, { status: 200 });
    }
...
export async function PUT(req) {
  ...
  try {
    ...
    const updated = await db
      .collection("category_banners")
      .findOne({ category: categoryDoc.name });

    logAudit(
      "CATEGORY_BANNER_UPLOADED",
      { userId: user.userId, userEmail: user.email, category },
      req,
    );
    return NextResponse.json({
      success: true,
      message: `Hero banner for "${category}" uploaded successfully`,
      banner: updated,
    });
  } catch (err) {
...
export async function DELETE(req) {
  ...
  try {
    ...
    if (!existing) {
      return NextResponse.json(
        { error: "Banner not found for this category" },
        { status: 404 },
      );
    }
    ...
    logAudit(
      "CATEGORY_BANNER_DELETED",
      { userId: user.userId, userEmail: user.email, category },
      req,
    );
    return NextResponse.json({
      success: true,
      message: `Hero banner for "${category}" removed successfully`,
    });
  } catch (err) {
