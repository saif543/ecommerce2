...
export async function GET(req) {
  try {
    ...
    const CACHE_KEY = `section_banners:${section}`;
    const CACHE_SECTION = await getRedisData(CACHE_KEY);
    if (CACHE_SECTION) {
      return NextResponse.json(CACHE_SECTION, { status: 200 });
    }
...
export async function PUT(req) {
  ...
  try {
    ...
    const uploaded = await uploadImage(buffer, {
      ...
      transformation: [
        { width: 1440, height: 160, crop: "fill", gravity: "auto" },
      ],
    });
    ...
    logAudit(
      "SECTION_BANNER_UPLOADED",
      { userId: user.userId, userEmail: user.email, section },
      req,
    );
    return NextResponse.json({
      success: true,
      message: `Banner for "${section}" uploaded successfully`,
      banner: updated,
    });
  } catch (err) {
...
export async function DELETE(req) {
  ...
  try {
    ...
    if (!existing)
      return NextResponse.json(
        { error: "Banner not found for this section" },
        { status: 404 },
      );
    ...
    logAudit(
      "SECTION_BANNER_DELETED",
      { userId: user.userId, userEmail: user.email, section },
      req,
    );
    return NextResponse.json({
      success: true,
      message: `Banner for "${section}" deleted successfully`,
    });
  } catch (err) {
