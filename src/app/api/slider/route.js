...
async function applySliderDiscounts(db, slideData) {
  ...
  if (hasSpecificProducts) {
    ...
    for (const entry of targetProducts) {
      ...
      let oid;
      try {
        oid = new ObjectId(String(entry.productId));
      } catch {
        continue;
      }

      const pct =
        parseFloat(entry.discountPercentage) > 0
          ? parseFloat(entry.discountPercentage)
          : globalPct;
      if (pct <= 0) continue;

      const product = await db
        .collection("products")
        .findOne({ _id: oid }, { projection: { price: 1 } });
      if (!product) continue;
    ...
    if (customOfferScope === "brand" && targetBrands?.length > 0) {
      ...
      customOfferScope === "category" &&
      targetCategories?.length > 0
    ) {
      query.category = {
        $in: targetCategories.map((c) => new RegExp(`^${c}$`, "i")),
      };
    } else {
    ...
    const bulkOps = products.map((p) => {
      const salePrice = parseFloat(
        (p.price * (1 - globalPct / 100)).toFixed(2),
      );
      return {
        updateOne: {
          filter: { _id: p._id },
...
export async function GET(req) {
  try {
    ...
    const CACHE_KEY = `slider:${includeInactive}:${slideId}`;
    const CACHE_SLIDER = await getRedisData(CACHE_KEY);
    if (CACHE_SLIDER) {
      return NextResponse.json(CACHE_SLIDER, { status: 200 });
    }
    ...
    const sliders = await db
      ...
      .sort({ order: 1 })
      .toArray();
    const result = { success: true, slides: sliders };
  ...
  try {
    ...
    if (action === "reorder") {
      if (!Array.isArray(slidesOrder)) {
        return NextResponse.json(
          { error: "slides array is required for reorder" },
          { status: 400 },
        );
      }
    ...
    if (action === "create") {
      ...
      // Offer fields
      const offerType = ["none", "custom"].includes(slideData.offerType)
        ? slideData.offerType
        : "none";
      const customOfferScope =
        offerType === "custom" &&
        ["brand", "category", "products"].includes(slideData.customOfferScope)
          ? slideData.customOfferScope
          : null;
      const targetBrands =
        customOfferScope === "brand" && Array.isArray(slideData.targetBrands)
          ? slideData.targetBrands
              .map((s) => sanitizeString(s, 200))
              .filter(Boolean)
          : [];
      const targetCategories =
        customOfferScope === "category" &&
        Array.isArray(slideData.targetCategories)
          ? slideData.targetCategories
              .map((s) => sanitizeString(s, 200))
              .filter(Boolean)
          : [];

      const targetProducts =
        offerType === "custom" && Array.isArray(slideData.targetProducts)
          ? slideData.targetProducts
              .slice(0, 200)
              .map((entry) => ({
                ...
                discountPercentage: Math.min(
                  100,
                  Math.max(0, parseFloat(entry.discountPercentage) || 0),
                ),
              }))
              .filter((e) => e.productId)
          : [];
      const globalDiscountPercentage = Math.min(
        100,
        Math.max(0, parseFloat(slideData.globalDiscountPercentage) || 0),
      );
      ...
      logAudit(
        "SLIDER_CREATED",
        { userId: user.userId, slideId: newSlide.id },
        req,
      );
      return NextResponse.json(
        {
    ...
    if (action === "update") {
      ...
      if (!slideId) {
        return NextResponse.json(
          { error: "Slide id is required for update" },
          { status: 400 },
        );
      }
      ...
      if (slideData) {
        ...
        if (slideData.isActive !== undefined)
          updateData.isActive = Boolean(slideData.isActive);
        ...
        if (slideData.offerType !== undefined) {
          updateData.offerType = ["none", "custom"].includes(
            slideData.offerType,
          )
            ? slideData.offerType
            : "none";
        ...
        const offerType =
          ...
          (await db.collection("sliders").findOne({ id: slideId }))
            ?.offerType ??
          "none";

        if (slideData.customOfferScope !== undefined) {
          updateData.customOfferScope = [
            "brand",
            "category",
            "products",
          ].includes(slideData.customOfferScope)
            ? slideData.customOfferScope
            : null;
        }
        if (
          slideData.targetBrands !== undefined &&
          Array.isArray(slideData.targetBrands)
        ) {
          updateData.targetBrands = slideData.targetBrands
            .map((s) => sanitizeString(s, 200))
            .filter(Boolean);
        }
        if (
          slideData.targetCategories !== undefined &&
          Array.isArray(slideData.targetCategories)
        ) {
          updateData.targetCategories = slideData.targetCategories
            .map((s) => sanitizeString(s, 200))
            .filter(Boolean);
        }

        if (
          slideData.targetProducts !== undefined &&
          Array.isArray(slideData.targetProducts)
        ) {
          updateData.targetProducts = slideData.targetProducts
            .slice(0, 200)
            .map((entry) => ({
              productId: String(entry.productId || ""),
              productName: sanitizeString(entry.productName || "", 300) || "",
        ...
        if (slideData.globalDiscountPercentage !== undefined) {
          updateData.globalDiscountPercentage = Math.min(
            100,
            Math.max(0, parseFloat(slideData.globalDiscountPercentage) || 0),
          );
        }
      ...
      await db
        .collection("sliders")
        .updateOne({ id: slideId }, { $set: updateData });
    ...
    return NextResponse.json(
      { error: "Invalid action. Use: create, update, or reorder" },
      { status: 400 },
    );
  } catch (err) {
...
export async function DELETE(req) {
  ...
  try {
    ...
    if (action === "toggle") {
      ...
      return NextResponse.json({
        success: true,
        message: `Slide ${slider.isActive ? "deactivated" : "activated"}`,
      });
    }
