...
export async function GET(req) {
  ...
  try {
    ...
    return createAuthError(
      authError.message,
      authError.message.includes("rate") ? 429 : 401,
    );
  }
  ...
  try {
    ...
    const CACHE_KEY = `user:${email}:${getAllUsers}:${page}:${limit}`;
    const CACHE_USER = await getRedisData(CACHE_KEY);
    if (CACHE_USER) {
      return NextResponse.json(CACHE_USER, { status: 200 });
    }
    ...
    if (email) {
      ...
      logAudit(
        "USER_EMAIL_CHECK",
        { userId: user.userId, checkedEmail: email, found: !!foundUser },
        req,
      );
      const result = {
        exists: !!foundUser,
        user: sanitizeUserResponse(foundUser),
      };
    ...
    if (getAllUsers === "true") {
      ...
      logAudit(
        "ALL_USERS_ACCESSED",
        { userId: user.userId, page, limit, total },
        req,
      );
      const result = {
    ...
    const result = { user: sanitizeUserResponse(currentUser) };
    await setRedisData(CACHE_KEY, result, R_1HRS);
    return NextResponse.json(result, { status: 200 });
  } catch (err) {
...
export async function POST(req) {
  ...
  try {
    ...
    if (action === "update") {
      ...
      if (phone !== undefined)
        updateData.phone = sanitizeString(String(phone), 20);
    ...
    try {
      authUser = await verifyApiToken(req);
      isAdminCreation =
        authUser.email.toLowerCase() !== body.email.toLowerCase();
    } catch {
      /* unauthenticated self-registration is allowed */
    }
    ...
    if (isAdminCreation) {
      try {
        ...
        return createAuthError(
          "Only admins can create accounts for other users",
          403,
        );
      }
    }
    ...
    const newUserData = {
      ...
      role:
        isAdminCreation && VALID_ROLES.includes(body.role) ? body.role : "user",
    ...
    logAudit(
      authUser ? "USER_CREATED_BY_ADMIN" : "USER_SELF_REGISTERED",
      {
        userId: authUser?.userId,
...
export async function PUT(req) {
  ...
  try {
    ...
    if (file.size > maxSize) {
      return NextResponse.json(
        {
          error: `File too large (max ${user.role === "admin" ? "10MB" : "5MB"})`,
        },
        { status: 400 },
      );
    }
    ...
    if (!existingUser) {
      ...
      await cloudinary.uploader
        .destroy(uploadResponse.public_id)
        .catch(() => {});
      return NextResponse.json({ error: "User not found" }, { status: 404 });
    }
    ...
    if (existingUser.profilePicturePublicId) {
      await cloudinary.uploader
        .destroy(existingUser.profilePicturePublicId)
        .catch(() => {});
    }
...
export async function DELETE(req) {
  ...
  try {
    ...
    await cloudinary.uploader
      .destroy(foundUser.profilePicturePublicId)
      .catch(() => {});
    await db.collection("user").updateOne(
      { _id: foundUser._id },
      {
    ...
    logAudit(
      ...
      { userId: user.userId, targetEmail: email },
      req,
    );
    return NextResponse.json({
      success: true,
