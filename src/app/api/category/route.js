...
export async function GET(req) {
  try {
    ...
    const CACHE_KEY = `category:${id}`;
    const CACHE_CATEGORY = await getRedisData(CACHE_KEY);
    if (CACHE_CATEGORY) {
      return NextResponse.json(CACHE_CATEGORY, { status: 200 });
    }
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      if (action === "add-subcategory") {
        ...
        const subcategoryName = sanitizeString(
        ...
        if (exists)
          return NextResponse.json(
            {
              error: `Subcategory "${subcategoryName}" already exists in this category`,
            },
            { status: 409 },
          );
        ...
        if ((category.subcategories || []).length >= MAX_SUBCATEGORIES) {
          return NextResponse.json(
            { error: `Max ${MAX_SUBCATEGORIES} subcategories per category` },
            { status: 400 },
          );
        }
        ...
        return NextResponse.json(
          {
            message: `Subcategory "${subcategoryName}" added`,
            category: updated,
          },
          { status: 201 },
        );
      ...
      if (action === "create-category") {
        ...
        logAudit(
          "CATEGORY_CREATED_NO_IMAGE",
          { userId: user.userId, userEmail: user.email, name },
          req,
        );
        return NextResponse.json(
          { message: "Category created successfully", category: created },
          { status: 201 },
        );
      }
  ...
  try {
    ...
    if (
      ...
      logAudit(
        ...
        { userId: user.userId, userEmail: user.email, categoryId: id },
        req,
      );
      return NextResponse.json({
    ...
    if (subcategoryId) {
      let subOid;
      try {
        subOid = new ObjectId(subcategoryId);
      } catch {
        return NextResponse.json(
      ...
      const subExists = (category.subcategories || []).some(
        (s) => s._id.toString() === subOid.toString(),
      );
      if (!subExists)
        return NextResponse.json(
      ...
      await db
        .collection("categories")
        .updateOne({ _id: oid }, { $set: { name, updatedAt: new Date() } });
      logAudit(
        "CATEGORY_RENAMED",
        {
          userId: user.userId,
...
export async function DELETE(req) {
  ...
  try {
    const { searchParams } = new URL(req.url);
    const id = sanitizeString(searchParams.get("id") || "", 100);
    const subcategoryId = sanitizeString(
    ...
    if (subcategoryId) {
      ...
      if (!force) {
        const count = await db.collection("products").countDocuments({
          subcategory: { $regex: `^${sub.name}$`, $options: "i" },
          isActive: true,
        });
        if (count > 0) {
          return NextResponse.json(
            {
              error: `Cannot delete — ${count} product(s) use this subcategory. Use ?force=true to override.`,
              productCount: count,
            },
            { status: 400 },
          );
      ...
      return NextResponse.json({
        message: `Subcategory "${sub.name}" deleted`,
      });
    }
    ...
    if (!force) {
      ...
      if (count > 0) {
        return NextResponse.json(
          {
            error: `Cannot delete — ${count} product(s) use this category. Use ?force=true to override.`,
            productCount: count,
          },
          { status: 400 },
        );
