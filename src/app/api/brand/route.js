...
export async function POST(req) {
    ...
    try {
        ...
        if (contentType.includes('multipart/form-data') || contentType.includes('application/octet-stream')) {
            ...
            if (logoFile && typeof logoFile !== 'string' && logoFile.size > MAX_IMAGE_SIZE) {
                return NextResponse.json({ error: 'Logo too large (max 100MB)' }, { status: 400 })
            }
            ...
            // Uniqueness check
            const existing = await db.collection('brands').findOne({ name: { $regex: `^${name}$`, $options: 'i' } })
        ...
        const db = client.db('ECOM2')
        const existing = await db.collection('brands').findOne({ name: { $regex: `^${name}$`, $options: 'i' } })
        if (existing) return NextResponse.json({ message: 'Brand already exists', brand: existing }, { status: 200 })
        ...
        logAudit('BRAND_CREATED_NO_LOGO', { userId: user.userId, userEmail: user.email, name }, req)
        return NextResponse.json({ message: 'Brand created successfully', brand: created }, { status: 201 })
...
export async function PUT(req) {
    ...
    try {
        ...
        if (contentType.includes('multipart/form-data') || contentType.includes('application/octet-stream')) {
            ...
            if (!logoFile || typeof logoFile === 'string') return NextResponse.json({ error: 'Logo file is required' }, { status: 400 })
            if (logoFile.size > MAX_IMAGE_SIZE) return NextResponse.json({ error: 'Logo too large (max 100MB)' }, { status: 400 })
            ...
            await db.collection('brands').updateOne(
                { _id: oid },
                { $set: { logo: logoUrl, logoPublicId: uploaded.publicId, updatedAt: new Date() } }
            )
            const updated = await db.collection('brands').findOne({ _id: oid })

            logAudit('BRAND_LOGO_UPDATED', { userId: user.userId, userEmail: user.email, brandId: id }, req)
            return NextResponse.json({ message: 'Brand logo updated', brand: updated })
        }
...
export async function DELETE(req) {
    ...
    try {
        ...
        if (brand.logoPublicId) {
            try { await deleteImage(brand.logoPublicId) } catch (e) { console.error('Cloudinary delete error:', e) }
        }
