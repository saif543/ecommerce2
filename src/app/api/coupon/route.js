...
function calculateDiscount(coupon, cartTotal, items) {
  ...
  if (
    ...
    applicableTotal = items
      .filter((i) =>
        coupon.productIds.includes(String(i.productId || i._id || i.id)),
      )
      .reduce((sum, i) => sum + i.price * (i.quantity || 1), 0);
    if (applicableTotal === 0) {
      return {
        valid: false,
        reason: "Coupon is not applicable to the products in your cart",
      };
    }
  } else if (
    (coupon.scope === "category" || coupon.scope === "subcategory") &&
    Array.isArray(items)
  ) {
    applicableTotal = items
      .filter((i) => {
...
export async function GET(req) {
  ...
  const CACHE_KEY = `coupon:${code}`;
  const CACHE_COUPON = await getRedisData(CACHE_KEY);
  if (CACHE_COUPON) {
    return NextResponse.json(CACHE_COUPON, { status: 200 });
  }
    ...
    try {
      ...
      if (!coupon)
        return NextResponse.json(
          { valid: false, reason: "Invalid or inactive coupon code" },
          { status: 404 },
        );
      ...
      await setRedisData(CACHE_KEY, finalResult, R_1HRS);
...
export async function POST(req) {
  ...
  try {
    ...
    if (existing)
      return NextResponse.json(
        { error: `Coupon code "${couponData.code}" already exists` },
        { status: 409 },
      );
    ...
    logAudit(
      "COUPON_CREATED",
      { userId: user.userId, userEmail: user.email, code: couponData.code },
      req,
    );
    return NextResponse.json(
      {
  ...
  try {
    ...
    if (newCode !== existing.code) {
      const conflict = await db
        .collection("coupons")
        .findOne({ code: newCode, _id: { $ne: oid } });
      if (conflict)
        return NextResponse.json(
          { error: `Coupon code "${newCode}" already exists` },
          { status: 409 },
        );
    }
...
function validateCouponBody(body) {
  ...
  if (!code || code.length < 2)
    return { error: "Coupon code is required (min 2 chars)" };
  if (!VALID_DISCOUNT_TYPES.includes(body.discountType))
    return {
  ...
  if (body.discountType === "percent" && val > 100)
    ...
    subcategories:
      scope === "subcategory"
        ? Array.isArray(body.subcategories)
          ? body.subcategories.filter(Boolean)
          : []
        : [],
    productIds:
      scope === "product"
